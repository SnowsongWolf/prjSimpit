...
void textAlign(Font t_font, string strText, Vector2 t_point, float t_size, float t_spacing, Color t_color,
  ...
  switch (al) {
    ...
    case A_CENTER:
      origin.x = t_area.x - tmpSize.x / 2 - (t_padding.right - t_padding.left)/2;  // I have a headache
      break;

    case A_RIGHT:
      origin.x = t_area.x - tmpSize.x - t_padding.left - t_padding.right;
      break;
  ...
  switch (val) {
    ...
    case V_CENTER:
      origin.y = t_area.y - tmpSize.y / 2 - (t_padding.bottom - t_padding.top)/2;
      break;

    case V_BOTTOM:
      origin.y = t_area.y - tmpSize.y - t_padding.top;
